            ...
            vector<vector<uint32_t>>&  backtrace_table)
{
    ...
    for (int c = 1; c < N; ++c) {
        ...
        // shared_p: bitmask of shared-read bit positions in prev's indexing.
        // shared_c: bitmask of shared-read bit positions in curr's indexing.
        uint32_t shared_p = 0, shared_c = 0;
        for (auto& [i, j] : shared) { shared_p |= (1u << i); shared_c |= (1u << j); }

        // ── proj table (size 2^Rp) ──────────────────────────────────────────────
        // Key = bp & shared_p  (no inner loop – one AND instruction).
        // Only 2^S of the 2^Rp entries are ever written; the rest stay INF.
        vector<unsigned>  proj       (1u << Rp, INF);
        vector<uint32_t>  proj_argmin(1u << Rp, 0);

        for (uint32_t bp = 0; bp < (1u << Rp); ++bp) {
            uint32_t key = bp & shared_p;               // ← user's change, correct here
        ...
        // ── Remap table (size 2^Rc, built in O(2^Rc)) ──────────────────────────
        // remap[bc] translates the shared-read bits of bc from curr's bit positions
        // to prev's bit positions, so it can index into proj directly.
        //
        // Recurrence (no inner loop):
        //   remap[0]   = 0
        //   remap[x]   = remap[x without its lowest set bit]
        //                | single_remap[position of lowest set bit]
        //
        // single_remap[j] = (1 << i) if curr position j maps to prev position i,
        //                   0        if j is not a shared position.
        ...
        for (uint32_t x = 1; x < (1u << Rc); ++x) {
            int lsb = __builtin_ctz(x);                 // position of lowest set bit
            remap[x] = remap[x ^ (1u << lsb)] | single_remap[lsb];
        }
        // remap[bc] == remap[bc & shared_c] for any bc (non-shared bits map to 0)
        ...
        for (uint32_t bc = 0; bc < (1u << Rc); ++bc) {
            uint32_t key  = remap[bc];                  // ← O(1), no inner loop
            unsigned prev = proj[key];
            if (prev < INF) {
                dp_table[c][bc]        = prev + col_cost(c, bc, cols[c], M);
